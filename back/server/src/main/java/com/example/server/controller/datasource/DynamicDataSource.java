...
//            }
    ...
    public boolean testDatasource(String key, String driveClass, String url, String username, String password) {
        try {
            Class.forName(driveClass);
            System.out.println("Class.forName(driveClass)成功了");
            //这一步有问题，经常输出“数据源配置有问题”，尽管参数都不为空
            DriverManager.getConnection(url, username, password);
            System.out.println("DriverManager.getConnection成功了");
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    private  boolean createDataSource(Datasource dataSource) throws Exception {
        ...
        if(databasetype.equals("mysql")){
            ...
            log.error("数据源"+datasourceId+"的driveClass为空");
        }
