...
public class ModelController {
    ...
    public RespBean save(@RequestBody Model file) throws IOException{
        try{
            if(modelMapper.selectOne(new QueryWrapper<Model>().eq("mname",file.getMname()).eq("version",file.getVersion()))!=null||
                modelMapper.selectOne(new QueryWrapper<Model>().eq("filename",file.getFilename()))!=null)
            {
