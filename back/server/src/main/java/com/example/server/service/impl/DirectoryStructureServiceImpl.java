...
public class DirectoryStructureServiceImpl implements IDirectoryStructureService {
    ...
    private final String baseDirPath=System.getProperty("user.dir") + "\\server\\src\\main\\resources\\files\\model\\";
     ...
     */
    @Override
    public List<String> getFileDirectoryStructure(String dirPath) {
...
//        filePaths.clear();
        ...
        for (File file : list) {
            if (file.isDirectory()) {
                System.out.println(createPrintStr(file.getName(), fileLevel));
                fileLevel++;
                // 递归子目录
                getFileDirectoryStructure(file.getPath());
                fileLevel--;
            } else {
                System.out.println(createPrintStr(file.getName(), fileLevel));
